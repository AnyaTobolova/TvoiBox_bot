...
- Удалить или архивировать старый dev/test-контур на VPS после периода наблюдения.
- Если dev-контур когда-нибудь снова понадобится, выделить для него отдельный домен и отдельного test bot; текущий `Deploy Dev Mini App` отключён и не должен поднимать `app.anyatobolova.ru`.
- Настроить CDN/proxy для `app` и `api`, если мобильные сети снова покажут нестабильность.
- Восстановить рабочий SSH-доступ без root-пароля и убрать зависимость от временного парольного входа.
- Перевыпустить production bot token и обновить secrets.
